...
print("Vorhandene Signale:", len(a1))
print("Vorhandene Signale nach Detektorempfindlichkeit:", len(a2))
...
for i in range(50000):
    ...
    x_korr1 = np.sqrt(1-rho**2) * 3 * tmp_x[0] + rho * 3 * tmp_y[0] + 7 # geht das so?
    y_korr1 = 3 * tmp_y[0] + 3
    x_korr2 = np.sqrt(1-rho**2) * 3 * tmp_x[1] + rho * 3 * tmp_y[1] + 7
    y_korr2 = 3 * tmp_y[1] + 3
...
#plt.xscale('log')
plt.xlabel(r'$E \,/\, \mathrm{TeV}$')
plt.savefig('1e_hits.pdf')
plt.clf()

plt.hist(x1, bins=50 ,alpha=0.8)
plt.title(r"X Ordinaten")
plt.xlabel(r'$x$')
plt.savefig('1e_x.pdf')
plt.clf()

plt.hist(y1, bins=50 ,alpha=0.8)
plt.title(r"Y Ordinaten")
plt.xlabel(r'$y$')
plt.savefig('1e_y.pdf')
plt.clf()
