...
plt.plot(x, f1(x), 'b-', label='Direkte Berechnung')
plt.xlim(0.999,1.001)
plt.ylim(-10**(-18),10**(-18))
plt.xlabel(r'$x$')
plt.ylabel(r'$f(x)$')
plt.legend(loc='best')
plt.tight_layout(pad=0, h_pad=1.08, w_pad=1.08)
...
plt.plot(x, f2(x), 'b-', label='Naive Berechnung')
plt.xlim(0.999,1.001)
plt.xlabel(r'$x$')
plt.ylabel(r'$f(x)$')
plt.legend(loc='best')
...
plt.plot(x, f3(x), 'b-', label='Hornerschema')
plt.xlim(0.999,1.001)
plt.xlabel(r'$x$')
plt.ylabel(r'$f(x)$')
plt.legend(loc='best')
...
plt.plot(x, g(x), 'b-', label='Numerische Annäherung')
plt.xscale('log')
plt.ylim(-0.45,0.05)
plt.xlabel(r'$x$')
plt.ylabel(r'$f(x)$')
...
def o(theta):
    return ((a**2)/s)*((2+np.sin(theta/360*2*np.pi)**2)/(1-(b**2)*np.cos(theta/360*2*np.pi)**2))

theta = np.linspace(0,10**(-5),10000)
plt.plot(theta, o(theta), 'b-', label='d.W.')
#plt.ylim(0, 10**(13))
plt.xlabel(r'$\theta$')
...
#def u(theta):
#    return ((a**2)/s)*((2+np.sin(theta/360*2*np.pi)**2)/(np.sin(theta/360*2*np.pi)**2+(1/y**2)*np.cos(theta/360*2*np.pi)**2))
#plt.plot(theta, u(theta), 'b-', label='d.W.')
##plt.ylim(0, 10**(13))
#plt.xlabel(r'$\theta$')
#plt.ylabel(r'$\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}$')
#plt.legend(loc='best')
